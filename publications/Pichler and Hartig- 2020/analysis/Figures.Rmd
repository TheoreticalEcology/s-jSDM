...
pdf(file = paste0("./figures/Fig_1",".pdf"), width = 9, height = 2.7)
...
for(i in (as.character(unique(number)))){
  ...
  #itle(paste0(as.numeric(i)*100, "% species"), line = 0.6, xpd = NA, font.main = 1)
...
## B
pdf(file = paste0("./figures/Fig_2",".pdf"), width = 6, height = 4.5)
...
### Figure S2 - Log Runtime
```{r}
pdf(file = paste0("./figures/Fig_S2",".pdf"), width = 9, height = 2.7)
...
pdf(file = paste0("./figures/Fig_3",".pdf"), width = 9, height = 4.9)
...
for(i in (as.character(unique(number)))){
 ...
 # title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  ...
  X = seq(1, nrow(setup), by = 3)+1.5
  ind = which(as.character(number) == i & setup$env == e, TRUE)
  mean_conf(X, gpuSparse$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[2], pch = 17)
  mean_conf(X, gllvmSparse$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[3], pc = 19)
  mean_conf(X, bcSparse$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[4], pch = 15)
  mean_conf(X, hmscSparse$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[5], pch = 4)
...
pdf(file = paste0("./figures/Fig_S3",".pdf"), width = 9, height = 4.9)
...
pdf(file = paste0("./figures/Fig_S4",".pdf"), width = 9, height = 9)
...
### Figure S5 - Covariance behaviour
Load results:
```{r}
setwd(here::here())
...
pdf(file = paste0("./figures/Fig_S5",".pdf"), width = 9, height = 4.3)
cols = RColorBrewer::brewer.pal(5, "Set2")
xx = 1:25
...
pdf(file = paste0("./figures/Fig_4",".pdf"), width = 9, height = 2.7)
e = 5L
lineT = rep(1,5) #1:5
names(lineT) =  (as.character(unique(number)))
...
pdf(file = paste0("./figures/Fig_S6",".pdf"), width = 9, height = 4.9)
...
# 11data_100_EL_51  
# 12data_100_EL_15  
pdf(file = paste0("./figures/Fig_S7",".pdf"), width = 9, height = 7)
cols = RColorBrewer::brewer.pal(5, "Set2")
# 1 3 5
cols = cols[c(2, 3, 5)]
ylim = c(0, 40)
...
```{r}
pdf(file = paste0("./figures/Fig_S8",".pdf"), width = 9, height = 7)
### Cov acc ###
ylim = c(0, 1)
par(mfrow = c(3,3), mgp = c(2.1,0.6,0), mar = c(0.6, 1.3, 0.3,0.7), oma = c(2.6,3.0,2.2,0)+0.3)
#### EL ####
...
pdf(file = paste0("./figures/Fig_S9",".pdf"), width = 9, height = 7)
### Cov rmse ###
ylim = c(0, 2)
par(mfrow = c(3,3), mgp = c(2.1,0.6,0), mar = c(0.6, 1.3, 0.3,0.7), oma = c(2.6,3.0,2.2,0)+0.3)
#### EL ####
...
```{r}
pdf(file = paste0("./figures/Fig_5",".pdf"), width = 8.88, height = 5.8) 
...
text(cos(deg2rad(123))*(lineSeq+0.7), sin(deg2rad(123))*(lineSeq+0.7), labels = "correlation", pos = 2, xpd = NA)
