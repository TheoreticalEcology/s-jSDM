...
gpu = readRDS(file = "./results/gpu_sjSDM_probit.RDS")
...
plot(NULL, NULL, xlim = c(0,1), ylim = c(0.0,6), xaxt = "n",xlab = "",main = "",yaxt = "n",  ylab = "runtime in minutes", xpd = NA,yaxs="i")
...
for(j in xx){
  for(i in unique(setup2$species)) {
      ...
      if(counter==1)axis(2, at = (tt/60),labels = c(round(tt/60,1)), las = 2, lwd = 0, lwd.ticks = 1, font = 1)
      if(i == "300") {
        if(j == 15000) d = 0.5
        else d = 0.3
...
if(!exists("gpu")) gpu = readRDS(file = "./results/gpu_sjSDM_probit.RDS")
...
for(k in 1:3) {
  for(j in seq(from = k, to = 27, by = 3)){
    ...
    for(i in 1:10) {
      ...
      raw = gpuSparse$auc[[j]][[i]]$pred$raw_weights
      ...
      gllvm_result_env[j,i] = mean(as.vector(t(gllvmSparse$auc[[j]][[i]]$pred$raw_weights) > 0) == as.vector(true> 0))
      gllvm_result_rmse[j,i] = sqrt(mean((as.vector(t(gllvmSparse$auc[[j]][[i]]$pred$raw_weights)) - as.vector(true))^2))
...
write.csv2(rbind(cov_acc, env_acc, env_rmse), "results/large_scale_results.csv")
```
...
```{r}
pdf(file = "./figures/Fig_LVM.pdf", width = 9, height = 4.0)
...
### Figure 5 new
```{r}
results = readRDS("results/tuning_fungi.RDS")
base_sigma = re_scale(getCov(results$base))
tuned_sigma = re_scale(getCov(results$model))
```
...
sigma = base_sigma[effect_comb_ind, effect_comb_ind]
sigmas = sigma[upper.tri(sigma)]
upper = order(sigmas, decreasing = TRUE)[1:number]
lower = order(sigmas, decreasing = FALSE)[1:number]
cuts = cut(sigmas, breaks = seq(-1,1,length.out = 12))
...
for(i in sort(unique(within))) {
  ...
  x = c((3.6+1.5*(sub_eff[,1]))*cos(deg2rad(sub[,3]) ), 
        rev((3.6+1.5/2)*cos(deg2rad(sub[,3]))))
  
  y = c((3.6+1.5*(sub_eff[,1]))*sin(deg2rad(sub[,3])),
        rev((3.6+1.5/2)*sin(deg2rad(sub[,3]))))
...
effects = coef(results$model)[[1]][,-1]
within = apply(effects, 1, function(e) which.max(abs(e)))
max_eff = sapply(1:length(within), function(i) effects[i,within[i]])
effect_comb = cbind(max_eff, within)
effect_comb_ind = order(within, max_eff)
