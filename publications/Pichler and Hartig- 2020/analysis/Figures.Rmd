...
mean_conf = function(sites, mat, col = "red", alpha = 0.1, spar = 0.10, convergence = NULL, indices = NULL, hmscObject = TRUE) {
...
par(mfrow = c(2,3), mgp = c(2.1,0.6,0), mar = c(4, 2.3, 4,0.7), oma = c(0.0,1.3,0.0,0))
...
for(i in (as.character(unique(number)))){
  ...
  plot(NULL, NULL, xlim = c(1+1.5,nrow(setup)-0.5), ylim = c(-0.5,3600), xaxt = "n", main = "",yaxt = "n",xlab = "Number of Sites", ylab = ylab, xpd = NA, font = 2)
  title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  if(i == "0.1") text(-0.03*(nrow(setup)-4), 3600*1.2, pos = 2, labels = "A", font = 2, xpd = NA, cex = 1.5)
  tt = seq(2, (3600), length.out = 10)
  tt = (tt)
  ...
  for(k in seq(1, nrow(setup), by = 3)[-1]){
    ...
    mean_conf(X, (cpu$result_time[as.character(number) == i & setup$env == e, ]), col = cols[1])
    mean_conf(X, (gpu$result_time[as.character(number) == i & setup$env == e, ]), col = cols[2])
    mean_conf(X, (gllvm$result_time[as.character(number) == i & setup$env == e, ]), col = cols[3])
    mean_conf(X, (bc$result_time[as.character(number) == i & setup$env == e, ]), col = cols[4],convergence = bc, hmscObject = FALSE, indices = ind)
    mean_conf(X, (hmsc$result_time[as.character(number) == i & setup$env == e, ]/2), col = cols[5], convergence = hmsc, indices = ind, hmscObject = TRUE)
  #legend("topleft", legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1, cex = 0.9, seg.len = 0.5, lwd = 2)
  
  if(i == "0.3") legend(x = -5, y = -1800,horiz = TRUE, legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
}
...
for(i in unique(setup2$species)) {
    ...
    plot(NULL, NULL, xlim = c(min(xx),max(xx)), ylim = c(0.0,max((large_scale$result_time[,1]))*1.1), xaxt = "n",xlab = "Number of sites x100",main = "",yaxt = "n",  ylab = ylab, xpd = NA)
    title(main = paste0(i, " species"), line = 0.6, xpd = NA)
    if(i == 300) text(x = min(xx) + 0.12*(min(xx)- max(xx)), y = diff(c(0.0,max((large_scale$result_time[,1]))*1.1))*1.2, pos = 2, labels = "B", xpd = NA, cex = 1.5, font = 2)
    for(k in seq(1, length(xx), by = 2)) abline(v = xx[k], col = addA("grey", 0.3))
    tt = seq(2.5, max((large_scale$result_time[,1]))*1.1, length.out = 6)
    tt = (tt)
    axis(1, at = xx[seq(1, length(xx), by = 2)], labels = xx[seq(1, length(xx), by = 2)]/100)
    axis(2, at = round((tt),2),labels = c(round(tt/60,0)), las = 2)
    for(k in tt) abline(h = (k), col = addA("grey", 0.3))
    lines(smooth.spline(x = xx, (large_scale$result_time[setup2$species == i]),spar = 0.5), lwd= lwd, lty = 1, col = cols[2])
...
## Figure 1 test alternative

```{r}
library(Hmsc)
library(coda)
cols = RColorBrewer::brewer.pal(5, "Set2")
pdf(file = "./figures/Fig_1_alternative.pdf", width = 9, height = 2.7)
...
for(i in (as.character(unique(number)))){
  ...
  # if(i == "0.3") legend(x = -5, y = -1800,horiz = TRUE, legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
   ...
   # legend("topleft",horiz = FALSE, legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
     legend("topleft", legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.0, pch = c(2, 17, 19, 15, 4))
}
...
## B
pdf(file = "./figures/Fig_1_large_alternative.pdf", width = 6, height = 4.5)
par(mfrow = c(1,1), mgp = c(2.1,0.6,0), mar = c(4, 2, 1,0.7), oma = c(0.0,1.3,0.0,0))
plot(NULL, NULL, xlim = c(0,1), ylim = c(0.0,11), xaxt = "n",xlab = "",main = "",yaxt = "n",  ylab = "runtime in minutes", xpd = NA,yaxs="i")
...
for(j in xx){
  for(i in unique(setup2$species)) {
      ...
      rect(xleft = coords[counter, 1], xright = coords[counter,2], ybottom = 0, ytop = large_scale$result_time[setup2$species == i & setup2$sites == j]/60,
           col = cols[2]) 
   
      axis(2, at = round((tt/60),2),labels = c(round(tt/60,0)), las = 2, lwd = 0, lwd.ticks = 1)
      if(i == "300") {
        text(x = coords[counter,2] + 0.04, y = large_scale$result_time[setup2$species == i & setup2$sites == j]/60 + 0.45 ,pos = 3, labels = paste0(site_labels[as.character(j)], " sites"))
      }
      
      text(x= coords[counter,1], y = -0.3, pos = 1, srt = 45, labels = species_labels[as.character(i)], xpd = NA)
      counter = counter + 1
  }
}
text(0.5, y = -1.4, pos = 1, label = "Species", xpd = NA)
    legend("topleft",horiz = FALSE, legend = c("gpu_dmvp"), col = cols[2], bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
dev.off()
...
if(!exists("gpuSparse")) gpuSparse = readRDS(file = "./results/sparse_gpu_dmvp.RDS")
...
if(!exists("baseSparse")) baseSparse = readRDS(file = "./results/sparse_gpu_dmvp_base.RDS")
if(!exists("data_sets2")) load("./data_sets.RData")
...
pdf(file = "./figures/Fig_2.pdf", width = 9, height = 7)
addA = function(col, alpha = 0.25) apply(sapply(col, col2rgb)/255, 2, function(x) rgb(x[1], x[2], x[3], alpha=alpha)) 

mean_conf = function(sites, mat, col = "red", alpha = 0.1, spar = 0.4, convergence = NULL, indices = NULL, hmscObject = TRUE, lambdasOnly = TRUE, lty = lty) {
  sites2 = sites[complete.cases(mat)]
  mat = mat[complete.cases(mat),]
  m = apply(mat, 1, mean)
  sd = apply(mat, 1, sd)
  ...
  if(!is.null(convergence)) {
    ...
    for(k in 1:length(indices)){
      ...
      lambdas = FALSE
      if(!hmscObject) {
        if(!lambdasOnly) betas = as.vector(sapply(convergence$post[[indices[k]]], function(p) 1.2 <abind::abind(p$psrf.beta,along = 1L)[,1]))
        if(lambdasOnly) lambdas = as.vector(sapply(convergence$post[[indices[k]]], function(p) 1.2 < p$psrf.gamma[,1]))
      }else {
        if(!lambdasOnly) betas = as.vector(sapply(convergence$post[[indices[k]]], function(p) 1.2 < p$psrf.beta[,1]))
        if(lambdasOnly) lambdas = unlist(sapply(convergence$post[[indices[k]]], function(p) 1.2 < gelman.diag(p$post$Lambda[[1]],multivariate = FALSE)$psrf[,1]))
      }
...
par(mfrow = c(3,3), mgp = c(2.1,0.6,0), mar = c(0.6, 2.3, 1,0.7), oma = c(4.0,1.3,2.2,0)+0.3)
number = setup$species
spar = 0.5
e = 5L
lwd = 2.2
...
e = 5L
  ...
  plot(NULL, NULL, xlim = c(1+1.5,nrow(setup)-0.5), ylim = c(0.5,1), xaxt = "n", yaxt = "n", xlab = "", ylab = ylab, xpd = NA)
  title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  ...
  for(k in seq(1, nrow(setup), by = 3)[-1]){
    ...
    mean_conf(X, bc$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[4],convergence = bc, hmscObject = FALSE, indices = ind, lambdasOnly = TRUE)
    mean_conf(X, hmsc$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[5],convergence = hmsc, hmscObject = TRUE, indices = ind, lambdasOnly = TRUE)
  }
...
for(i in (as.character(unique(number)))){
 ...
 # axis(3, at = seq(1, nrow(setup), by = 3)+1.5, labels = unique(setup$sites)* as.numeric(i))
  ...
  mean_conf(X, bc$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[4],convergence = bc, hmscObject = FALSE, indices = ind, lambdasOnly = FALSE)
  mean_conf(X, hmsc$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[5],convergence = hmsc, hmscObject = TRUE, indices = ind, lambdasOnly = FALSE)
}
...
for(i in (as.character(unique(number)))){
  ...
  plot(NULL, NULL, xlim = c(1+1.5,nrow(setup)-0.5), ylim = c(0.5,1), xaxt = "n", yaxt = "n", xlab = "Number of Sites", ylab = ylab, xpd = NA, xaxs = "r")
 # title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  if(i == "0.1") text(-0.05*(nrow(setup)-4), 1*1.05, pos = 2, labels = "C", font = 2, xpd = NA, cex = 1.5)
 ...
 # axis(3, at = seq(1, nrow(setup), by = 3)+1.5, labels = unique(setup$sites)* as.numeric(i))
  ...
  ind = which(as.character(number) == i & setup$env == e, TRUE)
#  mean_conf(X, cpu$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[1])
  mean_conf(X, gpuSparse$result_corr_auc[as.character(number) == i & setup$env == e, ], col = cols[2])
  #mean_conf(X, baseSparse$result_corr_auc[as.character(number) == i & setup$env == e, ], col = cols[2])

  mean_conf(X, gllvmSparse$result_corr_auc[as.character(number) == i & setup$env == e, ], col = cols[3])
  mean_conf(X, bcSparse$result_corr_auc[as.character(number) == i & setup$env == e, ], col = cols[4],convergence = bcSparse, hmscObject = FALSE, indices = ind, lambdasOnly = FALSE)
  mean_conf(X, hmscSparse$result_corr_auc[as.character(number) == i & setup$env == e, ], col = cols[5], convergence = hmscSparse, hmscObject = TRUE, indices = ind, lambdasOnly = FALSE)
  
   if(i == "0.3") legend(x = -5.5, y = 0.36,horiz = TRUE, legend = c( "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
...
pdf(file = "./figures/Fig_2_alternative.pdf", width = 11, height = 8.5)
addA = function(col, alpha = 0.25) apply(sapply(col, col2rgb)/255, 2, function(x) rgb(x[1], x[2], x[3], alpha=alpha)) 

mean_conf = function(sites, mat, col = "red", alpha = 0.1, spar = 0.4, convergence = NULL, indices = NULL, hmscObject = TRUE, lambdasOnly = TRUE, lty = lty, pch = 17) {
  sites2 = sites[complete.cases(mat)]
  mat = mat[complete.cases(mat),]
  m = apply(mat, 1, mean)
  sd = apply(mat, 1, sd)
  ...
  polygon(c(sites2, rev(sites2)), c(upper, rev(lower)), border = NA, col =addA(col, alpha))
  sp = smooth.spline(y = m, x = sites2, spar = spar)
  lines(sp, col = col, lwd = 2.0, type = "b", pch = pch)
...
e = 5L
  ...
  if(i == "0.1") text(-0.05*(nrow(setup)-4), 1*1.05, pos = 2, labels = "A", font = 2, xpd = NA, cex = 1.5)
    ...
    mean_conf(X, bc$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[4], pch = 15)#,convergence = bc, hmscObject = FALSE, indices = ind, lambdasOnly = TRUE)
    mean_conf(X, hmsc$result_corr_acc[as.character(number) == i & setup$env == e, ], col = cols[5], pch = 4)#,convergence = hmsc, hmscObject = TRUE, indices = ind, lambdasOnly = TRUE)
       legend("topleft", legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.0, pch = c(2, 17, 19, 15, 4))
  }
...
for(i in (as.character(unique(number)))){
  ...
  mean_conf(X, bc$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[4], pch = 15)#,convergence = bc, hmscObject = FALSE, indices = ind, lambdasOnly = FALSE)
  mean_conf(X, hmsc$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[5], pch = 4)#,convergence = hmsc, hmscObject = TRUE, indices = ind, lambdasOnly = FALSE)
     legend("topleft", legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.0, pch = c(2, 17, 19, 15, 4))
}
...
for(i in (as.character(unique(number)))){
 ...
 # title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  ...
  mean_conf(X, bcSparse$result_corr_auc[as.character(number) == i & setup$env == e, ], col = cols[4], pch = 19)#,convergence = bcSparse, hmscObject = FALSE, indices = ind, lambdasOnly = FALSE)
  mean_conf(X, hmscSparse$result_corr_auc[as.character(number) == i & setup$env == e, ], col = cols[5], pch = 15)#, convergence = hmscSparse, hmscObject = TRUE, indices = ind, lambdasOnly = FALSE)
  
   legend("topleft", legend = c("gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols[2:5], bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.0, pch = c(17, 19, 15, 4))
...
pdf(file = "./figures/Fig_S1.pdf", width = 9, height = 7)
...
e = 5L
  ...
  for(k in seq(1, nrow(setup), by = 3)[-1]){
    ...
    cat(i, "\n")
     X = seq(1, by = 3L,to = nrow(setup))+1.5
    ind = which(as.character(number) == i & setup$env == e, TRUE)
    mean_conf(X, cpu$result_env[as.character(number) == i & setup$env == e, ], col = cols[1])
    mean_conf(X, gpu$result_env[as.character(number) == i & setup$env == e, ], col = cols[2])
    mean_conf(X, gllvm$result_env[as.character(number) == i & setup$env == e, ], col = cols[3])
    mean_conf(X, bc$result_env[as.character(number) == i & setup$env == e, ], col = cols[4])
    mean_conf(X, hmsc$result_env[as.character(number) == i & setup$env == e, ], col = cols[5])
}
  ...
  for(j in seq(from = k, to = 27, by = 3)){
    ...
    for(i in 1:10) {
      ...
      tmp = which.max(sapply(gpuSparse$auc[[j]][[i]]$pred, function(rr) {
        ...
        raw = gpuSparse$auc[[j]][[i]]$pred[[tmp]]$raw_weights[[1]][[1]][[1]]
...
for(i in (as.character(unique(number)))){
  ...
  plot(NULL, NULL, xlim = c(1+1.5,nrow(setup)-0.5), ylim = c(0.5,1), xaxt = "n", yaxt = "n", xlab = "", ylab = ylab, xpd = NA, xaxs = "r")
  # title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  if(i == "0.1") text(-0.05*(nrow(setup)-4), 1*1.06, pos = 2, labels = "B", font = 2, xpd = NA, cex = 1.5)
 ...
 # axis(3, at = seq(1, nrow(setup), by = 3)+1.5, labels = unique(setup$sites)* as.numeric(i))
  ...
  mean_conf(X, gpu_result_env[as.character(number) == i & setup$env == e, ], col = cols[2])
  mean_conf(X, gllvm_result_env[as.character(number) == i & setup$env == e, ], col = cols[3])
  mean_conf(X, bc_result_env[as.character(number) == i & setup$env == e, ], col = cols[4])
  mean_conf(X, hmsc_result_env[as.character(number) == i & setup$env == e, ], col = cols[5])
}
...
for(i in (as.character(unique(number)))){
  ...
  # title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  if(i == "0.1") text(-0.05*(nrow(setup)-4), 1.3*1.06, pos = 2, labels = "C", font = 2, xpd = NA, cex = 1.5)
 ...
 # axis(3, at = seq(1, nrow(setup), by = 3)+1.5, labels = unique(setup$sites)* as.numeric(i))
  ...
  mean_conf(X, gpu_result_rmse[as.character(number) == i & setup$env == e, ], col = cols[2])
  mean_conf(X, gllvm_result_rmse[as.character(number) == i & setup$env == e, ], col = cols[3])
  mean_conf(X, bc_result_rmse[as.character(number) == i & setup$env == e, ], col = cols[4])
  mean_conf(X, hmsc_result_rmse[as.character(number) == i & setup$env == e, ], col = cols[5])
  
    if(i == "0.3") legend(x = -5.5, y = -0.24,horiz = TRUE, legend = c("gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols[2:5], bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
...
mean_conf(sites,mat = gpu_beh_adam$result_corr_acc, col = cols[2])
#mean_conf(gpu_beh_lbfgs$result_corr_acc, "black")
mean_conf(sites,gllvm_beh$result_corr_acc, col = cols[3])

mean_conf(sites,bc_beh$result_corr_acc,  convergence = bc_beh,hmscObject = FALSE, indices = ind , col = cols[4])
mean_conf(sites,hmsc_beh$result_corr_acc, convergence = hmsc_beh, hmscObject = TRUE, indices = ind,  col = cols[5],lambdasOnly = TRUE)

legend("topleft", legend = c("gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols[2:5], bty="n", lty = 1, cex = 0.9, seg.len = 0.5, lwd = 2)
...
for(i in seq(1, length(sites), by = 2)) abline(v = sites[i], col = addA("grey", 0.3))
for(i in seq(0.0, 1.0, 0.2)) abline(h = i, col = addA("grey", 0.3))
title("env RMSE",line = 1, xpd = NA)
axis(2, las = 2)
mean_conf(sites, gpu_beh_adam$result_rmse_env, col = cols[2])
#mean_conf(gpu_beh_lbfgs$result_env, "black")
mean_conf(sites, gllvm_beh$result_rmse_env, col = cols[3])
mean_conf(sites, bc_beh$result_rmse_env, col = cols[4])
mean_conf(sites, hmsc_beh$result_rmse_env, col = cols[5])
...
pdf(file = "figures/Fig_4_alternative.pdf", width = 9, height = 3.2)
e = 5L
lineT = rep(1,5) #1:5
names(lineT) =  (as.character(unique(number)))
...
for(i in (as.character(unique(number)))){
  ...
  cat(i, "\n")
  X = seq(1, by = 3L,to = nrow(setup))+1.5
  mean_conf(X, cpu_auc[as.character(number) == i & setup$env == e, ], col = cols[1])
  mean_conf(X, gpu_auc[as.character(number) == i & setup$env == e, ], col = cols[2])
  mean_conf(X, rbind(gllvm_auc, matrix(NA, length(number) - nrow(gllvm_auc) ,10L))[as.character(number) == i & setup$env == e, ], col = cols[3])
  mean_conf(X, rbind(bc_auc, matrix(NA, length(number) - nrow(bc_auc) ,10L))[as.character(number) == i & setup$env == e, ], col = cols[4])
  mean_conf(X, rbind(hmsc_auc, matrix(NA, length(number) - nrow(hmsc_auc) ,10L))[as.character(number) == i & setup$env == e, ], col = cols[5])
  if(i == "0.3") legend(x = -6, y =0.37,horiz = TRUE, legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
...
pdf(file = "figures/Fig_4_alternative.pdf", width = 9, height = 2.7)
e = 5L
lineT = rep(1,5) #1:5
names(lineT) =  (as.character(unique(number)))
...
for(i in 1:length(res_losses)){
  matplot(cbind(res_losses[[i]], apply(res_losses[[i]], 1, sum)), type = "o", pch = 18, lty = 1, col = 1:3, axes = FALSE, ylim = c(0,5000), xlab = "penalty", ylab = "losses", xpd = NA)
  text(x = -3, y = 6000, pos = 2, labels = nom[i], font = 2, xpd = NA)
  axis(1,at = 1:30, labels = format(lrs, scientific = TRUE, digits = 1), cex.axis = 0.7)
  axis(2, las = 2, cex.axis = 0.7)
  legend("topleft", legend = rev(c("overall", "w/o Weights", "Weights")), col = 1:3, lty = 1,bty = "n", cex = 0.7)
}
dev.off()
