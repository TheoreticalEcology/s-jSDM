...
```{r}
pdf(file = "./figures/Fig_2.pdf", width = 8, height = 5.5)
addA = function(col, alpha = 0.25) apply(sapply(col, col2rgb)/255, 2, function(x) rgb(x[1], x[2], x[3], alpha=alpha)) 
...
par(mfrow = c(2,3), mgp = c(2.1,0.6,0), mar = c(4.5, 2.3, 4.5,0.7), oma = c(0.0,1.3,0.0,0))
number = setup$species
spar = 0.5
e = 5L
lwd = 2.2
...
e = 5L
  ...
  for(k in seq(1, nrow(setup), by = 3)[-1]){
   ...
   #legend("topright", legend = c("gpu_dmvp", "cpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1, cex = 0.9, seg.len = 0.5, lwd = 2)
    if(i == "0.3") legend(x = -5, y = 0.33,horiz = TRUE, legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
  }
