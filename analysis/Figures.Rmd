...
```{r}
pdf(file = "./figures/Fig_2.pdf", width = 11, height = 9)
addA = function(col, alpha = 0.25) apply(sapply(col, col2rgb)/255, 2, function(x) rgb(x[1], x[2], x[3], alpha=alpha)) 

mean_conf = function(sites, mat, col = "red", alpha = 0.1, spar = 0.4, convergence = NULL, indices = NULL, hmscObject = TRUE, lambdasOnly = TRUE) {
  sites2 = sites[complete.cases(mat)]
  mat = mat[complete.cases(mat),]
  m = apply(mat, 1, mean)
  sd = apply(mat, 1, sd)
...
par(mfrow = c(3,3), mgp = c(2.1,0.6,0), mar = c(4, 2.3, 3,0.7), oma = c(0.7,1.3,0.2,0)+0.3)
number = setup$species
spar = 0.5
e = 5L
lwd = 2.2
...
e = 5L
  ...
  if(i == "0.1") text(-0.03*(nrow(setup)-4), 1*1.12, pos = 2, labels = "A", font = 2, xpd = NA, cex = 1.5)
...
for(i in (as.character(unique(number)))){
  ...
  title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  if(i == "0.1") text(-0.03*(nrow(setup)-4), 1*1.24, pos = 2, labels = "B", font = 2, xpd = NA, cex = 1.5)
...
for(i in (as.character(unique(number)))){
  ...
  plot(NULL, NULL, xlim = c(1+1.5,nrow(setup)-0.5), ylim = c(0.0,1), xaxt = "n", yaxt = "n", xlab = "Number of Sites", ylab = ylab, xpd = NA, xaxs = "r")
  title(paste0(as.numeric(i)*100, "% species"), line = 2, xpd = NA)
  if(i == "0.1") text(-0.03*(nrow(setup)-4), 1*1.24, pos = 2, labels = "C", font = 2, xpd = NA, cex = 1.5)
  ...
  ind = which(as.character(number) == i & setup$env == e, TRUE)
#  mean_conf(X, cpu$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[1])
  mean_conf(X, gpuSparse$result_corr_tss[as.character(number) == i & setup$env == e, ], col = cols[2])
  mean_conf(X, gllvmSparse$result_corr_tss[as.character(number) == i & setup$env == e, ], col = cols[3])
#  mean_conf(X, bc$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[4],convergence = bc, hmscObject = FALSE, indices = ind, lambdasOnly = FALSE)
 # mean_conf(X, hmsc$result_rmse_env[as.character(number) == i & setup$env == e, ], col = cols[5],convergence = hmsc, hmscObject = TRUE, indices = ind, lambdasOnly = FALSE)
  #legend("topright", legend = c("gpu_dmvp", "cpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1, cex = 0.9, seg.len = 0.5, lwd = 2)
   if(i == "0.3") legend(x = -5, y = -0.24,horiz = TRUE, legend = c("cpu_dmvp", "gpu_dmvp", "gllvm", "bayesComm", "Hmsc"), col = cols, bty="n", lty = 1,  seg.len = 0.5, lwd = 2, xpd = NA, cex = 1.0, x.intersp = 1.2)
}
