...
f = function(x) 2^x
lrs = f(lrs)
pdf(file = "./figures/Fig_S4_turn_over.pdf", width = 9, height = 4)
par(mfrow = c(1,1), mar = c(2, 3, 0.2, 2))
turn_over = abind::abind(turn_over_results, along = -1L)
matplot(t(turn_over), type = "o", pch = 18, lty = 1, col = 1:6, axes = FALSE, ylim = c(0,1))
axis(1,at = 1:30, labels = format(lrs, scientific = TRUE, digits = 1), cex.axis = 0.7)
axis(2, las = 2, cex.axis = 0.7)
legend("bottomright", legend = rates, col = 1:6, lty = 1,bty = "n", cex = 0.7)
dev.off()
```
...
effect_comb = cbind(max_effects[25,], sapply(1:n, function(i) result[[lr_step]]$beta[max_effects[lr_step,i],i] ))
