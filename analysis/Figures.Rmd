...
for(i in (as.character(unique(number)))){
  ...
  for(k in seq(1, nrow(setup), by = 3)[-1]){
    ...
    mean_conf(X, log(bc$result_time[as.character(number) == i & setup$env == e, ]), col = "green",convergence = bc, hmscObject = FALSE, indices = ind)
    mean_conf(X, log(hmsc$result_time[as.character(number) == i & setup$env == e, ]/2), col = "violet", convergence = hmsc, indices = ind, hmscObject = TRUE)
