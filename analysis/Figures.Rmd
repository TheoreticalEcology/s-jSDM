...
f = function(x) 2^x
lrs = f(lrs)
pdf(file = "./figures/Fig_5_new.pdf", width = 5.0, height = 6.66)

layout(matrix(c(1,2,3,rep(4,12)), 4,3 ,byrow = TRUE), c(1,1,1), c(1,1,1,1))
...
effect_comb = cbind(max_effects[25,], sapply(1:n, function(i) result[[25]]$beta[max_effects[25,i],i] ) )
effect_comb_ind = order(effect_comb[,1], effect_comb[,2])
effect_comb = effect_comb[effect_comb_ind, ]
#head(effect_comb[effect_comb_ind,])

sigma = re_scale(result[[25]]$sigma)[effect_comb_ind, effect_comb_ind]
sigmas = sigma[upper.tri(sigma)]
upper = order(sigmas, decreasing = TRUE)[1:number]
lower = order(sigmas, decreasing = FALSE)[1:number]
cuts = cut(sigmas, breaks = seq(-1,1,length.out = 12))
...
nseg = 100
plot(NULL, NULL, xlim = c(-5,5), ylim =c(-5,5),pty="s", axes = F, xlab = "", ylab = "")
text(x = -5, y = 5.7, pos = 3, xpd = NA, labels = "D", font = 2, cex = 1.5)
...
for(i in sort(unique(max_effects[1,]))) {
  ...
  # polygon(c(xx1, rev(xx2)), c(yy1, rev(yy2)),border = NA, col = cols[i], xpd = NA)
