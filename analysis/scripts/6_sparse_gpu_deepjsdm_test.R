...
for(i in 1:nrow(setup)) {
  ...
  for(j in 1:10){
    ...
    time = system.time({
      res_tmp = parLapply(cl,lrs, function(lambda) {
        ...
        model = layer_dense(model,ncol(train_Y),FALSE, FALSE, l1 = 0.7*lambda, l2 = 0.3*lambda)
        model = compileModel(model, nLatent = as.integer(tmp$species*tmp$sites),lr = 0.01,optimizer = "adamax",reset = TRUE, l1 = 0.7*lambda, l2 = 0.3*lambda,reg_on_Diag = FALSE,inverse = FALSE)
  ...
  saveRDS(gpu_dmvp, "results/sparse_gpu_dmvp_inv.RDS")
}
