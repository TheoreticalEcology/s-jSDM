...
JSDM account for correlation between species within communities (sites), in real datasets, however, communities (sites) are additionally correlated (== spatial autocorrelation).
...
# Fitting the latent variable model (still in development)
We also support fitting JSDM by the LVM via variational inference:
...
The model can be run either on the CPU or the GPU:
```{r,eval=FALSE}
lvm = sLVM(Y, X, family = binomial("probit"), device = "cpu")
summary(lvm)
```

Same functionalities as for sjSDM:
```{r,eval=FALSE}
...
Newdata, get posterior distribution of predictions:
```{r,eval=FALSE}
...
## Ordiation
model-based ordination based on the LVM:
```{r,eval=FALSE}
plot(lvm)
```
