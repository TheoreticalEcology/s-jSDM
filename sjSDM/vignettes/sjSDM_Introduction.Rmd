...
vignette: >
  ...
  %\VignetteIndexEntry{sjSDM: Getting started with the sjSDM package}
...
sjSDM has a function to create test data: We create a dataset with 3 environmental predictors, 100 species and 100 sites.
...
For convenience, we extract the community matrix, the environmental matrix and the spatial position

```{r}
Occ <- community$response
Env <- community$env_weights
SP <- matrix(rnorm(200, 0, 0.3), 100, 2) # spatial coordinates
```

## Fit model

The model is then fit with the function sjSDM(). You have to provide predictors (can be also a data.frame) and response as matrices. If you want/need p-values, you have to set se = TRUE.
...
As you see, for environmental and spatial predictors, you can provide the dependency you want via the standard R formular syntax. E.g. for an interaction between X1 and X2, write 

```{r, eval = F}
env = linear(data = Env, formula = ~ X1*X2,)
```

or for a quadratic effect of X1, but without an intercept, write

```{r, eval = F}
env = linear(data = Env, formula = ~0+ I(X1^2)+X1)
```
...
If you have species groups in your data, a bit nicer plot is produced by

```{r}
species=c("sp1","sp2","sp3","sp4","sp5","sp6","sp7","sp8","sp9","sp10")
group=c("mammal","bird","fish","fish","mammal","amphibian","amphibian", "fish","mammal","amphibian")
...
sp = x = 1:length(species)
image(x = sp, y = sp, z= association, )
```
