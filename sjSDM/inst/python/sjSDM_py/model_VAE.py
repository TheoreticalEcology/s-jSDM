...
class Model_VAE:
    ...
    def add_label_encoder(self, input_shape, output_shape, hidden=[], activation=['relu'], l1=-99, l2=-99):
        self.label_encoder = (self._build_NN(input_shape, output_shape, hidden, activation))
        self.params.append(self.label_encoder.parameters())
    ...
    def add_label_decoder(self, input_shape, output_shape, l1=-99, l2=-99):
        self.label_decoder = (self._build_NN(input_shape, output_shape, [], "", dropout_rate = -9))
        self.params.append(self.label_decoder.parameters())
        self.input_shape = input_shape
        self.output_shape = output_shape
        for p in self.label_decoder.parameters():
            if l1 > 0.0: 
                self.losses.append(lambda: self.l1_l2[0](p, l1))
            if l2 > 0.0:
                self.losses.append(lambda: self.l1_l2[1](p, l2))
    ...
    def add_feature_encoder(self, input_shape, output_shape, hidden = [], activation = ['relu'], l1=-99, l2=-99):
        #hidden.append(1)
        self.feature_encoder = (self._build_NN(input_shape, output_shape, hidden, activation))
        self.params.append(self.feature_encoder.parameters())
    ...
    def add_feature_decoder(self, input_shape, output_shape, l1=-99, l2=-99):
        self.feature_decoder = (self._build_NN(input_shape, output_shape, [], "", dropout_rate = -9))
        self.params.append(self.feature_decoder.parameters())
        self.input_shape = input_shape
        self.output_shape = output_shape
        for p in self.feature_decoder.parameters():
            if l1 > 0.0: 
                self.losses.append(lambda: self.l1_l2[0](p, l1))
            if l2 > 0.0:
                self.losses.append(lambda: self.l1_l2[1](p, l2))

    def add_decoder(self, input_shape, output_shape, hidden = [], activation = ['relu'], l1=-99, l2=-99):
        #hidden.append(1)
        self.decoder = (self._build_NN(input_shape, output_shape, hidden, activation))
        self.params.append(self.decoder.parameters())
    ...
    def _build_NN(self, input_shape, output_shape, hidden, activation, dropout_rate = 0.2):
    ...
    def build_multi_classify_loss(self, predictions, labels):
        ...
        truth_matrix = self.pairwise_and(y_i, y_not_i).type(self.dtype) 
        sub_matrix = self.pairwise_sub(predictions, predictions) 
        exp_matrix = torch.exp(torch.neg(5 * sub_matrix)) 

        sparse_matrix = torch.mul(exp_matrix, truth_matrix) 
        sums = torch.sum(sparse_matrix, dim=[2, 3]) 
        y_i_sizes = y_i.type(self.dtype).sum( dim=1) 
        y_i_bar_sizes = y_not_i.type(self.dtype).sum(dim=1) 
        normalizers = y_i_sizes.mul( y_i_bar_sizes)
        loss = sums.div( 5*normalizers) 
    ...
    def compute_BCE_and_RL_loss(self, E, input_label):
        sample_nll = torch.neg((torch.log(E)*input_label+torch.log(1-E)*(1-input_label)))
        logprob=-sample_nll.sum( dim=2)

        maxlogprob=logprob.max( dim=0).values
        Eprob=torch.exp(logprob-maxlogprob).mean( dim=0)
        nll_loss=torch.mean(-torch.log(Eprob)-maxlogprob)

        c_loss = self.build_multi_classify_loss(E, input_label)
        return nll_loss, c_loss

    def train_step(self, X, Y):
        eps1 = torch.tensor(1e-6, dtype=torch.float32)
        input_x = torch.cat([X, Y], dim  = 1)
        ##  Label Encoder
        le = self.label_encoder(input_x)
        fe_mu = le[:,0:self.latent_dim]
        fe_logvar = le[:, self.latent_dim:(self.latent_dim*2)]
        eps = torch.randn(size = fe_mu.shape)
        
        fe_sample = eps * torch.exp(fe_logvar / 2) + fe_mu

        ## Feature Encoder
        fe= self.feature_encoder(X)
        fx_mu = fe[:,0:self.latent_dim]
        fx_logvar = fe[:,self.latent_dim:(self.latent_dim*2)]
        eps = torch.randn(size = fx_mu.shape)
        fx_sample = eps * torch.exp(fx_logvar / 2) + fx_mu

        ## kl_loss
        kl_loss = torch.mean(0.5*torch.sum((fx_logvar-fe_logvar)-1+torch.exp(fe_logvar-fx_logvar)+torch.div(torch.pow(fx_mu-fe_mu, 2), torch.exp(fx_logvar)+1e-6), dim=1))

        c_fe_sample = torch.cat([X, fe_sample], 1)
        c_fx_sample = torch.cat([X, fx_sample], 1)

        ## Label Decoder
        label_mp_mu = self.label_decoder(self.decoder(c_fe_sample))
        ## Feature Decoder
        feat_mp_mu = self.feature_decoder(self.decoder(c_fx_sample))

        ## Losses
        noise = torch.randn(size=[self.n_sample, X.shape[0], self.z_dim])
        B = self.sigma.t()
        sample_r = torch.tensordot(noise, B, dims=1)+label_mp_mu 
        sample_r_x = torch.tensordot(noise, B, dims=1)+feat_mp_mu
        norm=torch.distributions.Normal(0., 1.)
        E = norm.cdf(sample_r)*(1-eps1)+eps1*0.5
        E_x = norm.cdf(sample_r_x)*(1-eps1)+eps1*0.5

        nll_loss, c_loss = self.compute_BCE_and_RL_loss(E, Y)
        nll_loss_x, c_loss_x = self.compute_BCE_and_RL_loss(E_x, Y)
        nll_coeff = 0.5
        c_coeff = 10.0
        total_loss = kl_loss*1.1 + (nll_loss + nll_loss_x) * nll_coeff +(c_loss + c_loss_x) * c_coeff
        #tf.reduce_sum(Label_encoder.losses) + tf.reduce_sum(Feature_encoder.losses)+tf.reduce_sum(Label_decoder.losses)+tf.reduce_sum(Feature_decoder.losses)
    ...
    def build(self, df=None,latent_dim=10, Re=None, optimizer=None, l1=0.0, l2=0.0,
