...
class Model_SVI():
    ...
    def build_model(self, X, Y,Re,SP, df, scale_mu = 5.0, scale_scale = 1.0, batch_size = 20):
        ...
        if Re is None:
            def model(XX, YY, link = self.link):
                ...
                with pyro.plate('data', size = XX.shape[0]) as ind:
                    loc_tmp = XX[ind,:].matmul(mu)
                    #loc_tmp = mu(XX[ind,:])
                    pyro.sample("obs", MultivariateProbit(loc_tmp, scale), obs = YY[ind,:])
        else:
            len_unique = np.unique(Re).shape[0]
            ...
            def model(XX, YY, link = self.link):
                ...
                with pyro.plate('data', size = XX.shape[0]) as ind:
                    loc_tmp = XX[ind,:].matmul(mu) + re.gather(0, indices[ind,:])
                    pyro.sample("obs", MultivariateProbit(loc_tmp, scale), obs = YY[ind,:])
    ...
    def fit(self, X, Y, df,RE=None, SP=None,
        ...
        if guide2 == 'LowRankMultivariateNormal':
            ...
            self.guide.add(pyro.infer.autoguide.AutoLowRankMultivariateNormal(pyro.poutine.block(self.model, expose=["mu"]), init_loc_fn=pyro.infer.autoguide.init_to_mean, init_scale=3.0 ))
            self.guide.add(pyro.infer.autoguide.AutoLowRankMultivariateNormal(pyro.poutine.block(self.model, expose=["scale"]),  init_loc_fn=pyro.infer.autoguide.init_to_mean, init_scale=0.01))
        ...
        if guide2 == 'DiagonalNormal':
            self.guide = pyro.infer.autoguide.AutoGuideList(self.model)
            self.guide.add(pyro.infer.autoguide.AutoDiagonalNormal(pyro.poutine.block(self.model, expose=["mu"]), init_loc_fn=pyro.infer.autoguide.init_to_mean, init_scale=3.0 ))
            self.guide.add(pyro.infer.autoguide.AutoDiagonalNormal(pyro.poutine.block(self.model, expose=["scale"]),  init_loc_fn=pyro.infer.autoguide.init_to_mean, init_scale=0.01))
            #guide = pyro.infer.autoguide.AutoDiagonalNormal(model, init_loc_fn=pyro.infer.autoguide.init_to_mean, init_scale=3.0)
        ...
        #    elbo = self.svi.step(XX, YY)
        #    if i % 100 == 0:
        #        logging.info("Elbo loss: {}".format(elbo))
        
        self.predictive = pyro.infer.Predictive(self.model, guide=self.guide, num_samples=num_samples)(torch.tensor(X, dtype=torch.float32), torch.tensor(Y, dtype=torch.float32))
        self.scale = self.predictive["scale"].data.squeeze().mean(dim=0).cpu()
        self.mu = self.predictive["mu"].data.squeeze().mean(dim=0).cpu()
        
        if RE is None:
            self.Re = None
        else:
            self.Re = self.predictive["re"].data.squeeze().mean(dim=0).cpu()
